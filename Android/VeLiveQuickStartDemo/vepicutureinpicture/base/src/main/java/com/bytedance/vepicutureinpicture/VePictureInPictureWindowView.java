...
public class VePictureInPictureWindowView extends CardView {

    public interface VePictureInPictureWindowViewObserver {
        default void onPositionChanged(int x, int y, int width, int height) {}
    }
    ...
    private VePictureInPictureWindowViewObserver mObserver;

    public VePictureInPictureWindowView(Context context) {
        this(context, null);
    }

    public VePictureInPictureWindowView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VePictureInPictureWindowView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setObserver(VePictureInPictureWindowViewObserver observer) {
        mObserver = observer;
    }
    ...
    public void requestWindowLayout() {
        ...
        if (mObserver != null) {
            mObserver.onPositionChanged(mParams.x, mParams.y, mParams.width, mParams.height);
        }
    }
    ...
    private static WindowManager.LayoutParams createPipLayoutParams() {
        ...
        // 窗口动画
        params.windowAnimations = R.style.ve_pip_animation;
        // 窗口 flag
    ...
    public void show() {
        ...
        if (mInitShow) {
            ...
            if (mParams.x <= 0) {
                mParams.x = mWindowInitX + mWindowMargin;
            } else if (mParams.x + mParams.width > metrics.widthPixels || mParams.x > mWindowMargin) {
                mParams.x = metrics.widthPixels - mParams.width - mWindowMargin;
            }
            if (mParams.y <= 0) {
                mParams.y = mWindowInitY + mWindowMargin;
            } else if (mParams.y + mParams.height > metrics.heightPixels) {
                mParams.y = metrics.heightPixels - mParams.height - mWindowMargin;
            }
    ...
    public void resetWindowSize(int width, int height) {
        ...
        int oriWidth = mParams.width;
        int oriHeight = mParams.height;

        if (oriHeight > 0 && oriWidth > 0) {
            if (mParams.x <= mWindowMargin) {
                //left
                mParams.y += ((oriHeight - height) / 2);
            } else {
                mParams.x += (oriWidth - width);
                mParams.y += ((oriHeight - height) / 2);

            }
        }
        if (mParams.x <= 0) {
            mParams.x = mWindowMargin;
        } else if (mParams.x + width > metrics.widthPixels || mParams.x > mWindowMargin) {
            mParams.x = metrics.widthPixels - width - mWindowMargin;
        }
        if (mParams.y <= 0) {
            mParams.y = mWindowMargin;
        } else if (mParams.y + height > metrics.heightPixels) {
            mParams.y = metrics.heightPixels - height - mWindowMargin;
        }
        ...
        mWindowInitWidth = width;
        mWindowInitHeight = height;
    }

    static class Wrapper {

        final VePictureInPictureWindowView mRef;

        Wrapper(VePictureInPictureWindowView ref) {
            this.mRef = ref;
        }
